...
(function ()
 {
   // undo function er kaj hochche jodi amra kono ekta drawing server e upload o kore feli chaile amra call
   //to action use kore undo kore felte pari
  "use strict";
  var SimpleUndo = function (options)
   {
    var settings = options ? options : {}; // ternary operation
    var defaultOptions =
     {
      provider: function () {
        throw new Error("No Image to display"); // ami kono e pachchi na tai exception throw korbo
      },
      maxLength: 30,
      onUpdate: function () {},
    };
...
DrawingBoard.Utils.pixelAt = function (image, x, y) {
  ...
  //cursor er x,y axis declatre koresi//
};
...
DrawingBoard.Board = function (id, opts) {
  ...
  this.dom = {
    ...
    $canvas: this.$el.find(".drawing-board-canvas"),//css part e jete hobe .drawing-board-canvas
    $cursor: this.$el.find(".drawing-board-cursor"),//css part e jete hobe .drawing-board-cursor
    $controls: this.$el.find(".drawing-board-controls"), //css part e jete hobe .drawing-board-controls
  };
...
DrawingBoard.Board.defaultOpts = {
  ...
  errorMessage:
    '<p>Browser Error. Update it</a> to start drawing.</p>',
  stretchImg: false, 
...
//inhistory function e amra jodi undo kori tahole amra ager drawing ki silo dekhte pabo
//redo korle amra porer drawing dekhte pabo//
  ...
  //drawing mode select korte hobe, amader project e 2 modes ase 1, newmode jokhon draw korbo tokhon
  //2, silent mode jokhon amra draw korbo na tokhon mouse er call to action
  ...
  /**
   * jei area te mouse stroke korbo sekhane dot draw hobe
   */
  fill: function (e) {
    ...
    var useHack = this.opts.fillHack; 
    ...
    while ((pixel = queue.pop())) {
      ...
      if (
        ...
        if (!useHack) updatePixelColor(pixel);
        if (pixel[X] > 0)
          // westside er dike koto tuku akbo
          queue.push(DrawingBoard.Utils.pixelAt(img, pixel[X] - 1, pixel[Y]));
        if (pixel[X] < maxX)
          // eastside er dike koto tuku draw korbo
          queue.push(DrawingBoard.Utils.pixelAt(img, pixel[X] + 1, pixel[Y]));
        if (pixel[Y] > 0)
          // northsider dik
  ...
  /**
   * Amader drawing kivabe korbo, mouse pad diye naki onno device diye. mouse sara amra draw korte parbo na
   * amra kebol e mouse diye draw korte parbo
   */
  ...
  draw: function () {
    //jodi pencil>10 hoy tahole , amader small_crosshair function would be called
    if (
...
//now amra directory extend korbo karon amader picture joto khushi toto hote pare//
